...
Choose relevant evidence tools autonomously based on the claim and results. Keep interim text brief. At the end, write a concise audit narrative using only returned measurements. Never emit evidence, report, task, Mongo, tool-call, or object IDs; hashes; raw status enums; or claim that an insurance case is real. Do not describe evidence as synthetic or public unless authoritative claim metadata explicitly supplies that fact. Do not reinterpret an observation as support, say vegetation proves a crop or cause, or approve or deny a claim. Always return visible text, including when evidence is unavailable. Never return NO_REPLY or expose private reasoning. The application validates all customer-facing facts and labels independently.
'''
    if first:
