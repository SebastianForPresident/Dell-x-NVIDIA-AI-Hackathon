...
export default function App() {
  ...
  async function runInvestigation() {
    ...
    try {
      ...
      for (let step = 0; step < 6; step++) {
        const result = await api(`/api/cases/${encodeURIComponent(caseId)}/demo-step`, { method: 'POST' })
        setCases(current => current.map(item => item.id === caseId ? result : item))
        if (result.status === 'READY_FOR_ADJUSTER_REVIEW' || result.status === 'NEEDS_EVIDENCE') break
  ...
  async function runReview() {
    if (!selected || !selected.report_saved) return
    ...
    {page !== 'home' && <main className="app-page">{loading ? <div className="page-state">Loading local cases…</div> : error ? <div className="page-state error"><Database size={30} /><h2>Backend unavailable</h2><p>{error}</p><p>Start MongoDB and FastAPI, then refresh.</p></div> : page === 'dashboard' ? <>
      ...
      <div className="claim-layout"><aside className="claim-context"><section className="surface"><div className="surface-heading"><div><span className="eyebrow">FIELD CONTEXT</span><h2>Claimed field</h2></div><MapPin size={19} /></div><FieldMap boundary={selected.boundary} selectedField={selected.selected_field} /><div className="context-facts"><div><span>Reported cause</span><strong>{selected.cause}</strong></div><div><span>Claimed crop</span><strong>{selected.crop}</strong></div><div><span>Loss date</span><strong>{dateLabel(selected.loss_date)}</strong></div></div></section><section className="surface"><span className="eyebrow">VEGETATION TREND</span><h2>Field NDVI</h2><MiniTrend points={selected.ndvi_series} /></section></aside>
        <section className="claim-main"><div className="surface investigation-panel">
          ...
          <p>The demo runs real local crop, rainfall, vegetation, and neighbor checks. Qwen can then interpret the saved structured results when OpenShell is connected.</p>
          <div className="progress-list">
            ...
            <div className={`progress-item ${reviewing ? 'active' : selected.ai_review ? 'done' : reviewError ? 'failed' : ''}`}><span>{selected.ai_review ? <Check size={16} /> : <Sparkles size={16} />}</span><div><strong>Qwen reviews structured evidence</strong><small>{reviewing ? 'Waiting for local Qwen through OpenShell…' : selected.ai_review ? 'Model interpretation saved' : 'Available after the measured report is saved'}</small></div></div>
          </div>
          ...
          <button className="button secondary run-button" onClick={runReview} disabled={reviewing || investigating || !selected.report_saved}><Sparkles size={18} /> {reviewing ? 'Qwen reviewing locally…' : selected.ai_review ? 'Run Qwen review again' : 'Run local Qwen review'}</button>
